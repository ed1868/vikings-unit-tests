describe('testing the sum function', () => {
  describe('testing the calculus', () => {
    it('if i pass two positive integers, i must receive their sum', () => {
      expect(sum(3, 4)).toBe(7)
    })
    it('if i pass two negative integers, i must receive their sum', () => {
      expect(sum(-3, -4)).toBe(-7)
    })
  })

  // describe('testing right parameters', () => {
  //   it ('if i pass an string as any of the params, it must throw an error', () => {
  //     var xx = sum(3, 'z')

  //     expect(xx).toThrowError("Both params must be a number")
  //   })

  // })
})

describe("Employee: this is a generic employee object which contains lalalala", () => {
  let emp1Mock = { n: "javi", a: 30, s: 0 }
  emp1Mock.s = 20000 + 10000 + 10000

  it("an object with the right params must be returned", () => {
    expect(employee('javi', 30)).toEqual(emp1Mock)
  })
  it("if the age is less than 20 years old, salary must be halved", () => {
    let emp1MockCopy = { ...emp1Mock, n: 'mario', a: 19, s: 20000 }
    expect(employee('mario', 19)).toEqual(emp1MockCopy)
  })
  it("if the age is less than 20 years old, salary must be 20000", () => {
    var emp1 = employee('mario', 19);

    expect(emp1['s']).toBe(20000)
  })
})

describe("TogglerComponent simply toggles .....", () => {
  var toggler

  beforeEach(() => {
    toggler = new TogglerComponent()
  })

  it('when instantiated I must received a false when retrieving its state', () => {
    expect(toggler.getState()).toEqual({ open: false })
  })
  it('when toggled, I must received a true in its state', () => {
    toggler.toggle()

    expect(toggler.getState()).toEqual({ open: true })
  })

  it('when toggled twice, I must received a false in its state', () => {
    toggler.toggle()
    toggler.toggle()
...
describe("salary calculator adds taxes to the raw salary", () => {
  it('checking that the salary gets it taxes added', () => {
    expect(salaryCalculator(10000)).toEqual(12100)
    expect(salaryCalculator(20000)).toEqual(24200)
    expect(salaryCalculator(30000)).toEqual(36300)
    expect(salaryCalculator(40000)).not.toEqual(10000)
  })

  it('checking that the salary at least gets something added', () => {
    expect(salaryCalculator(10000)).toBeGreaterThan(10000)
  })
})

describe("testing getDOMNode", () => {
  it('using spy', () => {
    ...
    expect(document.querySelector.calls.argsFor(0)[0]).toBe("body")

    
  })
})

// describe("these are the test for Soldier", function () {
//   var soldier;
//   var strength = 150;
//   var health = 300;
